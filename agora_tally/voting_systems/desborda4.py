...
# Desborda 4 is a modification/generalization of desborda 2.
# Basically, it's like Desborda 2, but without minorities corrections.

class Desborda4(BaseVotingSystem):
    '''
    ...
    @staticmethod
    def get_description():
        return _('Desborda 4 Count voting')
    ...
    def create_tally(election, question_num):
        ...
        '''
        return Desborda4Tally(election, question_num)

class Desborda4Tally(BaseTally):
    '''
    ...
    def get_max_points(self, question, desborda_kind):
        if 'desborda3' == desborda_kind:
            # if N is the number of winners, then the points start is
            # max_points = floor(N + 3N/10)
            return int(math.floor(question['num_winners'] + 3*question['num_winners']/10))
    ...
    def desborda_tally(self, question, ballots):
        ...
        max_points = self.get_max_points(question, 'desborda3')
